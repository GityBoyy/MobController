package org.chubby.github.mobcontroller.common.blocks.entity.fluid;
...
{
    public static final long BUCKET_CAPACITY = Services.FLUID_HELPER().getBucketCapacity();
    public static final long BOTTLE_CAPACITY = Services.FLUID_HELPER().getBottleCapacity();
    ...
    public final void sync(BlockEntity owner)
    {
        ...
        if(level != null && level.getChunkSource() instanceof ServerChunkCache cache)
        {
            ...
            players.forEach(player -> NetworkHandler.getPlay().sendToPlayer(() -> player, new MessageSyncFluid(pos, this.getStoredFluid(), this.getStoredAmount())));
        }
    }
    ...
    {
        return Services.FLUID_HELPER().createFluidContainer(capacity, null);
    }
    ...
    {
        return Services.FLUID_HELPER().createFluidContainer(capacity, onChange);
    }
}